...
export default {
  ...
  provideLinter: function() {
    ...
    const Shell = require('shell') // Electron's internal module
    return {
      name: 'ESLint',
      ...
      lint: textEditor => {
        ...
        return this.worker.request('JOB', {
          ...
          return response.map(function({message, line, severity, ruleId, column}) {
            ...
            let badge = Helpers.createElement('span')
            badge.setAttribute('class', 'badge badge-flexible')
            badge.innerHTML = ruleId || 'Fatal'
            let msgContainer = Helpers.createElement('span')
            msgContainer.innerHTML = ' '  + message

            if (ruleId) {
              badge.addEventListener('click', e => {
                if (e.target.className.includes('badge')) {
                  textEditor.setCursorBufferPosition(range[0])
                  return
                }
                Shell.openExternal('http://eslint.org/docs/rules/' + encodeURI(ruleId))
              })
            }
            if (showRule) {
              ret.html = ` ${escapeHTML(message)}`
            } else {
              ret.text = message
            }
            return ret
